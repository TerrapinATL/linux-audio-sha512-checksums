...
A Software Preflight script (Section 02 of the guide) verifies every required tool before any step runs and fails loudly with install hints if anything is missing.
...
`ALBUM.sha512sums.txt` — one line per track: `SHA512_HASH  filename.ext`, generated by `sha512sum`; verified with `sha512sum -c`.

`ARTIST.sha512sums.txt` — one line per album: `SHA512_HASH  Album Directory Name`. The stored hash is an aggregate (hash-of-hashes) computed recursively over the album's contents excluding `ALBUM.sha512sums.txt` itself. It must be verified using the Artist verification routine, which recreates each album hash and compares it. Do not use `sha512sum -c` on Artist manifests.
